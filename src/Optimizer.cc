...
int Optimizer::PoseOptimization(Frame *pFrame, Frame* pLastFrame, const IMUPreintegrator& imupreint, const cv::Mat& gw, const bool& bComputeMarg)
{
    ...
    g2o::EdgeNavStatePriorPVRBias* eNSPrior = new g2o::EdgeNavStatePriorPVRBias();
    {
        ...
        std::cout<<"prior information:\n"<<pLastFrame->mMargCovInv<<std::endl;
...
//cerr<<"after opt, iter: "<<it;
    ...
    std::cout<<"the velocity before optimization: "<<pFrame->GetNavState().Get_V().transpose()<<" norm: "<<pFrame->GetNavState().Get_V().transpose().norm()<<std::endl;
    std::cout<<"Time: "<<setprecision(16)<< pFrame->mTimeStamp <<" optimized velociy: "<<ns_recov.Get_V().transpose()<<" velocity norm: "<<ns_recov.Get_V().norm()<<std::endl;
    Vector3d incv = ns_recov.Get_V()-pFrame->GetNavState().Get_V();
    std::cout<<"increment: "<<incv.transpose()<<std::endl;
    if(fabs(incv(0))>0.3||fabs(incv(1))>0.3||fabs(incv(2))>0.3){
        std::cout<<"big increment on velocity!"<<std::endl;
    }
    ...
    std::cout<<"the last velocity before optimization: "<<pLastFrame->GetNavState().Get_V().transpose()<<" norm: "<<pLastFrame->GetNavState().Get_V().transpose().norm()<<std::endl;
    std::cout<<"Time: "<<setprecision(16)<< pLastFrame->mTimeStamp <<" optimized velociy: "<<nslast_recov.Get_V().transpose()<<" velocity norm: "<<nslast_recov.Get_V().norm()<<std::endl;
    std::cout<<"increment: "<<(nslast_recov.Get_V()-pLastFrame->GetNavState().Get_V()).transpose()<<std::endl;
...
//
        ...
        //Debug log
        cout<<"pFrame->mMargCovInv: \n"<<pFrame->mMargCovInv<<endl;
    }
...
int Optimizer::PoseOptimization(Frame *pFrame, KeyFrame* pLastKF, const IMUPreintegrator& imupreint, const cv::Mat& gw, const bool& bComputeMarg)
{
    ...
    g2o::EdgeNavStatePVR* eNSPVR = new g2o::EdgeNavStatePVR();
    {
        ...
        addcov.block<3,3>(6,6) *= 1e2;
        eNSPVR->setInformation(InvCovPVR + addcov);
        //std::cout<<"information:\n"<<(InvCovPVR + addcov).diagonal().transpose()<<std::endl;
        eNSPVR->SetParams(GravityVec);
