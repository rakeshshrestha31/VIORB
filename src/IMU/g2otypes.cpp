...
void EdgeNavStatePVR::computeError()
{
    ...
    double errorpvr = err.transpose()* _information.topLeftCorner(9,9)* err;
    std::cout << "error: " << err.transpose() << std::endl;
    std::cout << "information:\n" << _information.diagonal().transpose() << std::endl;
    std::cout << " errorpvr: "<<errorpvr<<std::endl;
...
void EdgeNavStatePVRPointXYZOnlyPose::linearizeOplus()
{
    ...
    Matrix3d Rcb = Rbc.transpose();
    Vector3d Pb = Rwb.transpose() * (Pw- Pwb);
    Vector3d Pc = Rcb * (Rwb.transpose() * (Pw- Pwb) - Pbc);
   ...
   // Matrix<double,2,3> JdRwb = - Jpi * (Sophus::SO3::hat(Paux) * Rcb);
    ...
    Matrix<double,2,3> JdRwb = - Jpi * Rcb * Sophus::SO3::hat(Rwb.transpose()*(Pw-Pwb));
...
//    err.segment<3>(6) = (Riprior_inv * nsPVRest.Get_R()).log();
    ...
    //Debug log
    std::cout<<"in optimization prior information(diagnoal):\n"<<_information<<std::endl;

    std::cout<<"prior edge error: "<<std::endl<<_error.transpose()<<std::endl;

    std::cout<<"prior edge error norm: "<<std::endl<<_error.transpose()* _information * _error <<std::endl;
